...
import { CandidateFilterDropdown, CandidateFilters } from './CandidateFilterDropdown'

export type SortField = 'name' | 'candidate_id' | 'expertise_name' | 'field_name' | 'area_name' | 'headhunter_name'
...
const sortOptions: { field: SortField; label: string }[] = [
  { field: 'name', label: 'Tên ứng viên' },
  { field: 'candidate_id', label: 'Mã ứng viên' },
  { field: 'expertise_name', label: 'Chuyên môn' },
  { field: 'field_name', label: 'Lĩnh vực' },
  { field: 'area_name', label: 'Khu vực' },
  { field: 'headhunter_name', label: 'Nhân sự phụ trách' },
]
    ...
    <div className="bg-white rounded-lg shadow-md p-6 mb-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-1">
          {/* Search Bar */}
          <div className="relative flex-1 max-w-3xl" ref={searchBarRef}>
            {loading ? (
            ...
            <Input
              type="text"
              placeholder="Tìm kiếm theo tên ứng viên, email, hoặc số điện thoại..."
              value={searchQuery}
            ...
            {Object.values(filters).some(value => value !== undefined) && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <div className="w-2 h-2 bg-[#982B1C] rounded-full"></div>
              </div>
            )}
